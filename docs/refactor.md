...
- Extended the Shopify UCP catalog fallback so variant-level `checkout_url` values are preserved as checkout evidence and runnable `payment_probe_candidates`. This lets mainstream Shopify/UCP sites land in a stronger `checkout_redirect` boundary instead of stopping at plain `catalog_only`.
...
- Added `build_rerun_slice.py` so we can emit a smaller CSV from prior receipt shards for targeted rescans. The default slice matches the current 1M plan: `catalog_surface`, `callable_surface`, `rate_limited`, plus domains with strong commerce hints.
